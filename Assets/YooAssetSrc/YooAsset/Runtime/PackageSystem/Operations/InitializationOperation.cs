...
namespace YooAsset
{
    ...
    internal sealed class EditorSimulateModeInitializationOperation : InitializationOperation
    {
        ...
        internal EditorSimulateModeInitializationOperation(
            EditorSimulateModeImpl impl,
        ...
        internal override void Update()
        {
            if (_steps == ESteps.LoadEditorManifest)
            {
                ...
                {
                    _loadEditorManifestOp = new LoadEditorManifestOperation(
                        _simulateManifestFilePath
                    );
    ...
    internal sealed class OfflinePlayModeInitializationOperation : InitializationOperation
    {
        ...
        internal OfflinePlayModeInitializationOperation(
            OfflinePlayModeImpl impl,
            string packageName
        )
        {
            _impl = impl;
        ...
        internal override void Update()
        {
            ...
            {
                LogMaster.S("[YooAssetPipeline]", " QueryBuildinPackageVersion");
                if (_queryBuildinPackageVersionOp == null)
                {
                    //! buildin  ？？  查询Package 版本
                    _queryBuildinPackageVersionOp = new QueryBuildinPackageVersionOperation(
                        _packageName
                    );
            ...
            if (_steps == ESteps.LoadBuildinManifest)
            {
                ...
                {
                    _loadBuildinManifestOp = new LoadBuildinManifestOperation(
                        _packageName,
                        _queryBuildinPackageVersionOp.PackageVersion
                    );
    ...
    internal sealed class HostPlayModeInitializationOperation : InitializationOperation
    {
        ...
        internal HostPlayModeInitializationOperation(HostPlayModeImpl impl, string packageName)
        {
            LogMaster.Log(
                "[YooAssetPipeline]",
                " HostPlayModeInitializationOperation init Op packageName: " + packageName
            );
            _impl = impl;
            _packageName = packageName;
        }
        ...
        internal override void Update()
        {
            ...
            if (_steps == ESteps.CheckAppFootPrint)
            {
                ...
                if (appFootPrint.IsDirty() || debugFootPrint)
                {
                    ...
                    LogMaster.S(
                        "[CheckAppFootPrint]",
                        "删除本地清单文件，并覆盖 FootPrint,   Delete manifest files when application foot print dirty !"
                    );
                }
                else
                {
            ...
            if (_steps == ESteps.QueryCachePackageVersion)
            {
                if (_queryCachePackageVersionOp == null)
                {
                    LogMaster.S(
                        "[YooAssetPipeline]",
                        "查询 Package版本  QueryCachePackageVersion query"
                    );
                    /// <summary>
                    ///！ 查询AB Package 版本
                    /// </summary>
                    _queryCachePackageVersionOp = new QueryCachePackageVersionOperation(
                        _packageName
                    );
            ...
            if (_steps == ESteps.TryLoadCacheManifest)
            {
                if (_loadCacheManifestOp == null)
                {
                    ...
                    _loadCacheManifestOp = new LoadCacheManifestOperation(
                        _packageName,
                        _queryCachePackageVersionOp.PackageVersion
                    );
            ...
            if (_steps == ESteps.QueryBuildinPackageVersion)
            {
                if (_queryBuildinPackageVersionOp == null)
                {
                    LogMaster.S("[YooAssetPipeline]", "QueryBuildinPackageVersion query build in");
                    _queryBuildinPackageVersionOp = new QueryBuildinPackageVersionOperation(
                        _packageName
                    );
                    OperationSystem.StartOperation(_queryBuildinPackageVersionOp);
            ...
            if (_steps == ESteps.UnpackBuildinManifest)
            {
                ...
                {
                    LogMaster.S("[YooAssetPipeline]", " UnpackBuildinManifest ");
                    _unpackBuildinManifestOp = new UnpackBuildinManifestOperation(
                        _packageName,
                        _queryBuildinPackageVersionOp.PackageVersion
                    );
            ...
            if (_steps == ESteps.LoadBuildinManifest)
            {
                if (_loadBuildinManifestOp == null)
                {
                    LogMaster.S("[YooAssetPipeline]", " LoadBuildinManifest ");
                    _loadBuildinManifestOp = new LoadBuildinManifestOperation(
                        _packageName,
                        _queryBuildinPackageVersionOp.PackageVersion
                    );
                ...
                if (_loadBuildinManifestOp.Status == EOperationStatus.Succeed)
                {
                    ...
                    LogMaster.S(
                        "[YooAssetPipeline]",
                        "[PackageRuntimeData] 设定版本号，  succeed version:" + PackageVersion
                    );
                }
                else
                {
            ...
            if (_steps == ESteps.PackageCaching)
            {
                if (_cachingOperation == null)
                {
                    LogMaster.S(
                        "[YooAssetPipeline] ",
                        " PackageCaching  cache _packageName:" + _packageName
                    );
                    _cachingOperation = new PackageCachingOperation(_packageName);
                    OperationSystem.StartOperation(_cachingOperation);
                }
    ...
    internal class AppFootPrint
    {
        ...
        public void Load()
        {
            ...
            if (File.Exists(footPrintFilePath))
            {
                ...
                LogMaster.Log(
                    "[AppFootPrint] ",
                    "读取沙盒中的 footPrint" + _footPrint + "  from: " + footPrintFilePath
                );
            }
            else
            {
        ...
        public bool IsDirty()
        {
            ...
            LogMaster.S(
                "[FootPrint]    ",
                $" check IsDirty  ,  Compare local.footPrint:{_footPrint}  version:{Application.version} "
            );
            ...
            LogMaster.I(
                "[CheckFootPrint]    ",
                $" _footPrint:{_footPrint}  Application.buildGUID:{Application.buildGUID} "
            );
        ...
        public void Coverage()
        {
            ...
            LogMaster.Log(
                "[FootPrint]",
                $" 本地保存 版本号 ，写入到本地文本内。 Save application foot print : {_footPrint}   path：{footPrintFilePath}"
            );
        }
    }
}
