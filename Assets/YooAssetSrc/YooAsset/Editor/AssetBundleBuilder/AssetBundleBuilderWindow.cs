...
namespace YooAsset.Editor
{
    ...
    public class AssetBundleBuilderWindow : EditorWindow
    {
        ...
        public static void BuildInternal(Action callback, ECopyBuildinFileOption buildOpt = ECopyBuildinFileOption.None)
        {
            BuildTarget buildTarget = EditorUserBuildSettings.activeBuildTarget;
            LogMaster.BP($"开始构建 AssetBundle : {buildTarget}");
            ...
            buildParameters.PackageName = VersionSettings.PackageName;
            ...
            buildParameters.CopyBuildinFileOption = buildOpt;

            // 执行构建
            AssetBundleBuilder builder = new AssetBundleBuilder();
            var buildResult = builder.Run(buildParameters);

            LogMaster.BP($"构建 AssetBundle Over ");

            if (buildResult.Success)
            {
                LogMaster.BP($"AssetBundle 构建成功 : {buildResult.OutputPackageDirectory}");

                if (callback != null)
                {
                    LogMaster.BP($" Build 首包文件------------》  ");
                    callback.Invoke();
                }
            }
            else
            {
                LogMaster.BP($" Error:  构建失败 : {buildResult.FailedTask}");
            }
        }
