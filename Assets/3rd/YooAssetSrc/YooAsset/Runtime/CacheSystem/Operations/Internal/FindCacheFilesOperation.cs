...
namespace YooAsset
{
    ...
    internal class FindCacheFilesOperation : AsyncOperationBase
    {
        ...
        /// 需要验证的元素
        /// </summary>
        public readonly List<VerifyCacheElement> VerifyElements = new List<VerifyCacheElement>(
            5000
        );
        ...
        internal override void Update()
        {
            ...
            if (_steps == ESteps.FindPrepare)
            {
                // BundleFiles
                {
                    ...
                    LogMaster.S(
                        "[YooAssetPipeline]",
                        $" FindPrepare BundleFiles rootPath:{rootPath}  "
                    );
                ...
                // RawFiles
                {
                    ...
                    LogMaster.S(
                        "[YooAssetPipeline]",
                        $" FindPrepare RawFiles rootPath:{rootPath}  "
                    );
            ...
            if (_steps == ESteps.FindRawFiles)
            {
                ...
                LogMaster.Log(
                    "[YooAssetPipeline]",
                    $"FindCacheFiles   查找Cache 文件 completed!!   Find cache files elapsed time {costTime:f1} seconds"
                );
            }
        }
        ...
        private bool UpdateFindBundleFiles()
        {
            ...
            while (true)
            {
                ...
                foreach (var chidDirectory in childDirectories)
                {
                    ...
                    string dataFilePath =
                        $"{fileRootPath}/{YooAssetSettings.CacheBundleDataFileName}";
                    string infoFilePath =
                        $"{fileRootPath}/{YooAssetSettings.CacheBundleInfoFileName}";

					LogMaster.S("[YooAssetPipeline]",$"VerifyCache \n  fileRootPath {fileRootPath} \n dataFilePath {dataFilePath}  infoFilePath  {infoFilePath} ");
					
                    VerifyCacheElement element = new VerifyCacheElement(
                        _packageName,
                        cacheGUID,
                        fileRootPath,
                        dataFilePath,
        ...
        private bool UpdateFindRawFiles()
        {
            ...
            while (true)
            {
                ...
                foreach (var chidDirectory in childDirectories)
                {
                    ...
                    string fileRootPath = chidDirectory.FullName;
                    string dataFilePath =
                        $"{fileRootPath}/{YooAssetSettings.CacheBundleDataFileName}{dataFileExtension}";
                    string infoFilePath =
                        $"{fileRootPath}/{YooAssetSettings.CacheBundleInfoFileName}";
                    VerifyCacheElement element = new VerifyCacheElement(
                        _packageName,
